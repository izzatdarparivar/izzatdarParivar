...
export default function ChatSidebar({ userId }: ChatSidebarProps) {
  ...
  const filteredSessions = sessions.reduce((acc, current) => {
    const otherUid = getOtherParticipantId(current, userId);
    const existing = acc.find(s => getOtherParticipantId(s, userId) === otherUid);
    
    if (!existing) {
      acc.push(current);
    } else {
      const currentDate = current.updatedAt?.toDate() || new Date(0);
            ...
            {filteredSessions.map((session) => {
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-3 p-3.5 rounded-2xl transition-all duration-200 group mx-1 ${
                    isActive
                ...
                >
                  <div className="relative flex-shrink-0">
                    <div className={`w-11 h-11 rounded-full flex items-center justify-center text-white text-base font-serif font-bold shadow-sm transition-transform group-hover:scale-105 ${isActive ? "gold-gradient" : "bg-orange-200/50 text-orange-800"}`}>
                      {initial}
                    </div>
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      <h3 className={`text-[13px] font-bold truncate transition-colors ${isActive ? "text-orange-900" : "text-[var(--on-surface)]"}`}>
                        {displayName}
                      </h3>
                      <span className={`text-[9px] font-medium shrink-0 ${isActive ? "text-orange-600" : "text-gray-400"}`}>
                        {timeStr}
                      </span>
                    </div>
                    <p className={`text-[11px] truncate leading-relaxed ${isActive ? "text-orange-800/70" : "text-[var(--on-surface-variant)]"}`}>
                      {lastMsg}
                    </p>
                  </div>
                </Link>
